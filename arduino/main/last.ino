...
float feats_buffer[NUMBER_OF_INPUTS];

int raw_feature_get_data(size_t offset, size_t length, float *out_ptr) {
    memcpy(out_ptr, feats_buffer + offset, length * sizeof(float));
    return 0;
}
...
void loop() {
  ...
  if (idx >= 100) {
    // 1️⃣ Extract features from buffered IMU data
    extractFeatures(axBuf, ayBuf, azBuf, gxBuf, gyBuf, gzBuf, 100, feats_buffer);

    // 2️⃣ Build Edge Impulse signal using the static buffer approach
    signal_t features_signal;
    features_signal.total_length = NUMBER_OF_INPUTS;
    ...
    // 4️⃣ Retrieve and display probability
